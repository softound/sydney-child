...
register_widget( 'Sydney_Testimonials_Scroller' );
}

class Sydney_Testimonials_Scroller extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'sydney_testimonials_scroller_widget', 'description' => __( 'Display your testimonials in scroller.', 'sydney') );
        parent::__construct(false, $name = __('Sydney FP: Testimonials Scroller', 'sydney'), $widget_ops);
		$this->alt_option_name = 'sydney_testimonials_scroller_widget';
    }
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['sydney_testimonials_scroller']) )
			delete_option('sydney_testimonials_scroller');		  
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'sydney_testimonials_scroller', 'widget' );
		}
...
?>
			...
			<div class="col-md-12">
				<div class="roll-testimonials-scroller" data-autoplay="<?php echo intval($autoplay); ?>">
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'sydney_testimonials_scroller', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
...
add_action('wp_enqueue_scripts', 'sy_testimonial_scroller_scripts' );
add_action('wp_head', 'sy_testimonial_scroller_css_js' );


function sy_testimonial_scroller_scripts() {
	wp_enqueue_style( 'sy-slick', get_template_directory_uri().'-child/assets/slick/slick.css');
	wp_enqueue_style( 'sy-slick-theme', get_template_directory_uri().'-child/assets/slick/slick-theme.css');
	wp_enqueue_script( 'sy-slick-js', get_template_directory_uri().'-child/assets/slick/slick.min.js', array('jquery'));
}

function sy_testimonial_scroller_css_js(){
?>
	<script>
	jQuery(document).ready(function(){
		jQuery('.roll-testimonials-scroller').slick({
		slidesToShow: 4,
		slidesToScroll: 4,
		responsive: [
			{
